...
const getPreItem = express.Router();
...
getPreItem.post('/getPreItem', async (req, res) => {
    const tab = req.body.tab
    try {

    
        if(tab === 'all'){
            var data = await preItemMaster.findAll({})
        }else{
            var data = await preItemMaster.findAll({where:{channel:tab}})
        }
